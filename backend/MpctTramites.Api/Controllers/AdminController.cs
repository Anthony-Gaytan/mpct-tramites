...
public sealed class AdminController(AppDbContext db, UserManager<Usuario> users):ControllerBase
{
 ...
 [HttpPut("tarifas/{tipo}")] public async Task<IActionResult> UpdateTariff(TipoSolicitud tipo,ActualizarTarifaRequest request,CancellationToken ct){if(request.Monto<=0||request.Monto>10000)return BadRequest(new{message="El importe debe ser mayor que cero y menor o igual a S/ 10,000."});var tarifa=await db.Tarifas.SingleOrDefaultAsync(x=>x.Tipo==tipo&&x.Activa,ct);if(tarifa is null)return NotFound();tarifa.Monto=decimal.Round(request.Monto,2);db.Auditorias.Add(new Auditoria{UsuarioId=CurrentUserId(),Accion="ACTUALIZAR_TARIFA",Entidad="Tarifa",EntidadId=tarifa.Id.ToString(),DetalleJson=System.Text.Json.JsonSerializer.Serialize(new{tipo,monto=tarifa.Monto})});await db.SaveChangesAsync(ct);return Ok(new{tarifa.Id,tarifa.Tipo,tarifa.Monto});}
 ...
 public async Task<IActionResult> CreateUser(CrearPersonalRequest request)
 {
   ...
   if(await users.FindByEmailAsync(email) is not null)return BadRequest(new{message="Ya existe una cuenta registrada con este correo. Usa un correo diferente para el cajero o inspector."});
 ...
 public async Task<IActionResult> ChangeStatus(Guid id,CambiarEstadoUsuarioRequest request)
 {
   ...
   var currentUserId=CurrentUserId();if(currentUserId==id&&!request.Activo)return BadRequest(new{message="No puedes desactivar tu propia cuenta de administrador."});
   ...
   user.Activo=request.Activo;user.MotivoSuspension=request.Activo?null:request.Motivo!.Trim();user.SuspendidoEn=request.Activo?null:DateTimeOffset.UtcNow;user.SuspendidoPorId=request.Activo?null:currentUserId;
 ...
 {
   var id=CurrentUserId();var user=await users.FindByIdAsync(id.ToString());if(user is null)return Unauthorized();
 ...
 private Guid CurrentUserId(){var raw=User.FindFirstValue(ClaimTypes.NameIdentifier)??User.FindFirstValue("sub");return Guid.TryParse(raw,out var id)?id:throw new UnauthorizedAccessException("La sesión no contiene un identificador válido. Vuelve a iniciar sesión.");}
}

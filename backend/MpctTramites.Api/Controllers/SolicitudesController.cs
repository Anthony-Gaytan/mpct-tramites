...
public sealed class SolicitudesController(AppDbContext db, SunatService sunat, IWebHostEnvironment env) : ControllerBase
{
    ...
    public async Task<IActionResult> Create(CrearSolicitudRequest request, CancellationToken ct)
    {
        ...
        var userId=Guid.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier),out var authenticatedId)?authenticatedId:(Guid?)null;
        var solicitud=new Solicitud { NumeroExpediente=$"MPCT-{DateTime.UtcNow:yyyy}-{RandomNumberGenerator.GetInt32(100000,999999)}", CodigoSeguimientoHash=Hash(code), Empresa=empresa, CiudadanoId=userId, Tipo=request.Tipo, RepresentanteNombre=request.RepresentanteNombre, RepresentanteDocumento=request.RepresentanteDocumento, RepresentanteEmail=request.RepresentanteEmail, Rubro=request.Rubro, Actividad=request.Actividad, AreaMetrosCuadrados=request.AreaMetrosCuadrados, DireccionLocal=request.DireccionLocal, Tarifa=tarifa, Estado=EstadoSolicitud.PendientePago };
        solicitud.Historial.Add(new HistorialEstado{Estado=solicitud.Estado,Comentario="Solicitud registrada"}); db.Solicitudes.Add(solicitud); await db.SaveChangesAsync(ct); return CreatedAtAction(nameof(Get),new{id=solicitud.Id},new{solicitud.Id,solicitud.NumeroExpediente,uploadToken=code,solicitud.Estado,solicitud.Tarifa});
    }
    ...
    [HttpPost("seguimiento"), EnableRateLimiting("sensitive")] public async Task<IActionResult> Track(SeguimientoRequest r,CancellationToken ct) { var x=await db.Solicitudes.AsNoTracking().Include(s=>s.Empresa).Include(s=>s.Historial).Where(s=>s.Empresa!.Ruc==r.Ruc).OrderByDescending(s=>s.CreadoEn).FirstOrDefaultAsync(ct); return x is null?NotFound(new{message="No se encontró un expediente para el RUC indicado."}):Ok(new{x.NumeroExpediente,x.Estado,x.CreadoEn,x.Observaciones,historial=x.Historial.OrderByDescending(h=>h.CreadoEn)}); }
    [HttpPost("{id:guid}/documentos")][RequestSizeLimit(10_485_760)] public async Task<IActionResult> Upload(Guid id,[FromForm]IFormFile archivo,[FromQuery]string tipo,[FromQuery]string? codigo,CancellationToken ct) { var extension=Path.GetExtension(archivo.FileName).ToLowerInvariant();var allowedExtensions=new[]{".pdf",".jpg",".jpeg",".png"}; if(!allowedExtensions.Contains(extension)||archivo.Length<=0||archivo.Length>10_485_760)return BadRequest(new{message="Solo PDF, JPG o PNG de hasta 10 MB."}); var solicitud=await db.Solicitudes.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id,ct);if(solicitud is null)return NotFound();var authenticated=Guid.TryParse(User.FindFirstValue(ClaimTypes.NameIdentifier),out var uid)&&(solicitud.CiudadanoId==uid||User.IsInRole("ADMINISTRADOR"));if(!authenticated&&(string.IsNullOrWhiteSpace(codigo)||solicitud.CodigoSeguimientoHash!=Hash(codigo)))return Unauthorized(new{message="La autorización para adjuntar el documento no es válida."}); var folder=Path.Combine(env.ContentRootPath,"storage",id.ToString());Directory.CreateDirectory(folder);var name=$"{Guid.NewGuid():N}{extension}";var path=Path.Combine(folder,name);await using(var f=System.IO.File.Create(path))await archivo.CopyToAsync(f,ct);await using var read=System.IO.File.OpenRead(path);var hash=Convert.ToHexString(await SHA256.HashDataAsync(read,ct));var doc=new Documento{SolicitudId=id,Tipo=tipo,NombreOriginal=Path.GetFileName(archivo.FileName),NombreAlmacenado=name,Mime=archivo.ContentType,Tamano=archivo.Length,Sha256=hash};db.Documentos.Add(doc);await db.SaveChangesAsync(ct);return Ok(new{doc.Id,doc.NombreOriginal}); }

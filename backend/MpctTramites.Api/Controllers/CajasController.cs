...
{
 [HttpGet("estado")] public async Task<IActionResult> Status(CancellationToken ct){var uid=UserId();var caja=await db.Cajas.AsNoTracking().Where(x=>x.CajeroId==uid&&x.EstaAbierta).OrderByDescending(x=>x.AbiertaEn).FirstOrDefaultAsync(ct);return Ok(new{abierta=caja is not null,caja});}
 [HttpGet("solicitud/{ruc}")] public async Task<IActionResult> FindRequest(string ruc,CancellationToken ct){var item=await db.Solicitudes.AsNoTracking().Include(x=>x.Empresa).Where(x=>x.Empresa!.Ruc==ruc&&x.Estado==EstadoSolicitud.PendientePago).OrderByDescending(x=>x.CreadoEn).Select(x=>new{x.Id,x.NumeroExpediente,x.Tarifa,x.Estado,razonSocial=x.Empresa!.RazonSocial,ruc=x.Empresa.Ruc}).FirstOrDefaultAsync(ct);return item is null?NotFound(new{message="No existe una solicitud pendiente de pago para este RUC."}):Ok(item);}

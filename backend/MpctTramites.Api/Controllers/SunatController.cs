...
{
    [HttpGet("ruc/{ruc}")] public async Task<IActionResult> Validate(string ruc, CancellationToken ct) { var (data,error)=await sunat.ValidateAsync(ruc,ct); return error is null ? Ok(new { data!.Ruc, data.RazonSocial, data.Direccion, data.Ubigeo, data.Estado, data.Condicion, fuente="Consulta directa SUNAT", validadoEn=DateTimeOffset.UtcNow }) : BadRequest(new { message=error }); }

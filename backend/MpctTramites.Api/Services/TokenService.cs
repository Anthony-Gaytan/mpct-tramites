...
public sealed class TokenService(IConfiguration config)
{
    public string Create(Usuario user, IEnumerable<string> roles)
    {
        ...
        var claims = new List<Claim> { new(JwtRegisteredClaimNames.Sub, user.Id.ToString()), new(JwtRegisteredClaimNames.Email, user.Email!), new(ClaimTypes.Name, $"{user.Nombres} {user.Apellidos}") };
